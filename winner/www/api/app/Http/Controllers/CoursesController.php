...
class CoursesController extends Controller
{
    ...
    public function store(Request $request, _SecurityController $_security)
    {
        ...
        if ($is_success) {
            ...
            // $authentication_data = $oFunc->authentication('/live_events');
            ...
            // try {
            //     $response = $client->request('POST', env('LIVE_TRANSCODE_API').'/live_events', [
            //         'headers' => [
            ...
            // try {
            //     $responseCretateDirectory = $httpClient->request('POST', env('BASE_URL_API_MEDIA').'directories/create_dir.php', [
            //         'json' => $params
            //     ]);
            ...
            if ($data->groupsConnectRegis->count() > 0) {
                ...
                try {
                    ...
                    if ($e->hasResponse()) {
                        ...
                        return response()->json(["message" => $clientErr['error_msg']." - (R)", "params" => $paramCourse], $e->getResponse()->getStatusCode());
                    }
    ...
    public function show($id, _RolesController $oRole)
    {
        ...
        if ($authSession->super_users) {
            ...
            $data->categories = $data->categories(function($query) use ($authSession) {
                $query->where('admins_id', $authSession->id)->orWhere(function($subQuery) use ($authSession) {
                    $subQuery->where('groups_id', $authSession->groups_id)->whereNull('admins_id');
                });
            })->get();
    ...
    public function update($id, Request $request, _SecurityController $_security, _RolesController $oRole)
    {
        ...
        if (is_array($input['course2category'])) {
            ...
            if ($authSession->super_users) {
                ...
                $dataCategories = $data->categories(function($query) use ($authSession) {
                    $query->where('admins_id', $authSession->id)->orWhere(function($subQuery) use ($authSession) {
                        $subQuery->where('groups_id', $authSession->groups_id)->whereNull('admins_id');
                    });
                })->get();
    ...
    public function updateStatus($id, Request $request, _SecurityController $_security, _RolesController $oRole)
    {
        //
        $input = $request->json()->all();
    ...
    public function allInGroups($course_id = null)
    {
        //
        $data = new Courses;
        $authSession = Auth::user();
        $authSessionGroups = $authSession->admins_groups->groups;

        if($authSession->super_users){
            $data = $data->where(function($query) use ($authSession) {
                $query->where('admins_id', $authSession->id)->orWhere('level_public', 1);
            });
        ...
        if (is_numeric($course_id)) {
            $dataCourse = Courses::find($course_id);
            $arrExcept = array($course_id);

            if ($dataCourse) {
                foreach ($dataCourse->related as $related) {
                    $arrExcept[] = $related->id;
                }
            }

            $data = $data->whereNotIn('id', $arrExcept);
        }

        $data = $data->orderBy('order', 'asc')->get();
        for($i=0; $i<count($data); $i++) {
            $data[$i]->code_title = $data[$i]->code." - ".$data[$i]->title;
        }
        return response()->json($data, 200);
    ...
    public function overview($id)
    {
        ...
        $data = Courses::find($id);
        $data->groups = $data->groups()->get();
        $data->categories = $data->categories()->with('groups')->get();
        $data->instructors = $data->instructors()->get();
        ...
        $data->documents = $data->documents()->where('status', 1)->get();

        $data->pre_test = $data->quiz()->where('type', 1)->where('status', 1)->first();
        if(count($data->pre_test)){
            $data->pre_test->questions = $data->pre_test->questions()->where('status', 1)->orderBy('order', 'asc')->get();
        ...
        if(count($data->post_test)){
            $data->post_test->questions = $data->post_test->questions()->where('status', 1)->orderBy('order', 'asc')->get();
        ...
        if(count($data->exam)){
            $data->exam->questions = $data->exam->questions()->where('status', 1)->orderBy('order', 'asc')->get();
    ...
    public function allExcept($id)
    {
        ...
        if ($dataCourse) {
            $dataCourse->related = $dataCourse->related()->get();
            foreach ($dataCourse->related as $related) {
                $arrExcept[] = $related->id;
            }
        }

        $data = Courses::whereNotIn('id', $arrExcept)->orderBy('order', 'asc')->get();
        // $data = Courses::orderBy('order', 'asc')->get();
        for($i=0; $i<count($data); $i++) {
            $data[$i]->code_title = $data[$i]->code." - ".$data[$i]->title;

            // if (in_array($data[$i]->id, $arrExcept)) {
    ...
    public function topics($id, Request $request) {
        $data = Courses::find($id)->topics()->whereNull('topics.parent')->orderBy($request['order_by'],$request['order_direction'])->paginate($request['per_page']);
        for($i=0; $i<count($data); $i++) {
            $admins = Admins::find($data[$i]->modify_by);
            $data[$i]->no = $i + 1;
            $data[$i]->modify_by = $admins->username;
    ...
    public function getIncomingStream($id) {

        $courses_data = Courses::find($id);

        $client = new httpClient();

        try {
            $response = $client->request('GET', env('WOWZA_URL_API').'servers/_defaultServer_/vhosts/_defaultVHost_/applications/'.$courses_data['streaming_applications'].'/instances/_definst_', [
        ...
        $data = $this->namespacedXMLToArray($response->getBody());

        return response()->json($data, 200);

    }

    public function getIncomingStreamDuration($id) {
        ...
        try {
            $response = $client->request('GET', env('WOWZA_URL_API').'servers/_defaultServer_/vhosts/_defaultVHost_/applications/'.$courses_data['streaming_applications'].'/instances/_definst_/streamrecorders/'.$courses_data['streaming_prefix_streamname'], [
                'auth' => [env('WOWZA_USERNAME'), env('WOWZA_PASSWORD'), env('WOWZA_AUTH_TYPE')]
        ...
        if ($courses_data['is_stop_record'] == 0) {
            $courses_data['current_duration_record'] = $data['CurrentDuration'];
        } else {
            $courses_data['current_duration_record'] += $data['CurrentDuration'];
        }

        return response()->json($courses_data['current_duration_record'], 200);

    }

    public function getIncomeDuration($id) {
    ...
    public function startRecord($id) {
        ...
        $params = array(
            ...
            // "recorderName" => $courses_data['streaming_streamname'],
            "recorderName" => $courses_data['streaming_prefix_streamname'],
            "startOnKeyFrame" => true,
            "outputPath" => "/raid/original/".$courses_data['streaming_record_part'],
            "applicationName" => $courses_data['streaming_applications'],
            "moveFirstVideoFrameToZero" => true,
            "defaultRecorder" => true,
            "splitOnTcDiscontinuity" => false,
            "baseFile" => $courses_data['streaming_record_filename'].".mp4",
            "RecordData" => false,
            "SplitOnTcDiscontinuity" => false,
            "BackBufferTime" => 0,
            "fileTemplate" => $courses_data['streaming_record_filename'].".mp4",
        ...
        try {
            $response = $client->request('POST', env('WOWZA_URL_API').'servers/_defaultServer_/vhosts/_defaultVHost_/applications/'.$courses_data['streaming_applications'].'/instances/_definst_/streamrecorders', [
                'auth' => [env('WOWZA_USERNAME'), env('WOWZA_PASSWORD'), env('WOWZA_AUTH_TYPE')],
                'json' => $params
            ]);
        } catch(RequestException $e) {
    ...
    public function stopRecord($id) {
        ...
        try {
            $responseDuration = $client->request('GET', env('WOWZA_URL_API').'servers/_defaultServer_/vhosts/_defaultVHost_/applications/'.$courses_data['streaming_applications'].'/instances/_definst_/streamrecorders/'.$courses_data['streaming_prefix_streamname'], [
                'auth' => [env('WOWZA_USERNAME'), env('WOWZA_PASSWORD'), env('WOWZA_AUTH_TYPE')]
        ...
        if ($courses_data['is_stop_record'] == 0) {
            $courses_data['is_stop_record'] = 1;
        }

        $courses_data['current_duration_record'] += $dataDuration['CurrentDuration'];
        $courses_data->save();

        try {
            $response = $client->request('PUT', env('WOWZA_URL_API').'servers/_defaultServer_/vhosts/_defaultVHost_/applications/'.$courses_data['streaming_applications'].'/instances/_definst_/streamrecorders/'.$courses_data['streaming_prefix_streamname'].'/actions/stopRecording', [
                'auth' => [env('WOWZA_USERNAME'), env('WOWZA_PASSWORD'), env('WOWZA_AUTH_TYPE')]
            ]);
        } catch(RequestException $e) {
            if ($e->hasResponse()) {
    ...
    public function slidesActive($id) {
        ...
        // $firstSlide = Courses::find($id)->slides()->orderBy('order', 'asc')->first();
        // if ($data->id == $firstSlide->id) {
        //     $data->isFirstSlide = true;
        // } else {
        //     $data->isFirstSlide = false;
        // }
        
        return response()->json($data, 200);
    }

    public function firstSlide($id) {
        //
        $data = Courses::find($id)->slides()->orderBy('order', 'asc')->first();
        // $firstSlide = Courses::find($id)->slides()->orderBy('order', 'asc')->first();
        // if ($data->id == $firstSlide->id) {
        //     $data->isFirstSlide = true;
        // } else {
    ...
    public function discussions($id, Request $request) {
        //
        $data = Courses::find($id)->discussions()->with(['courses', 'members' => function($query) {
            $query->select('id', 'first_name', 'last_name');
        }])->whereNull('parent_id')->orderBy($request['order_by'],$request['order_direction'])->paginate($request['per_page']);
        for($i=0; $i<count($data); $i++) {
            $data[$i]->replies = Discussions::where('parent_id', $data[$i]->id)->get();
            ...
            for($j=0; $j<count($data[$i]->replies); $j++) {
                $data[$i]->replies[$j]->replies = Discussions::where('parent_id', $data[$i]->replies[$j]->id)->get();
