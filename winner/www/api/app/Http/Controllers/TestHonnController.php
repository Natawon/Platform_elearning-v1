...
class TestHonnController extends Controller
{
    ...
    public function register(Request $request, SiteController $_site)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'password' => 'required|between:8,255|case_diff|numbers|letters|not_contain_credentials:'.$request['email'],
        ...
        if ($_user) {
            ...
            if($groupsKey == $_user->groups->key){
                if($_user->level_groups){
                    ...
                    // exit;
                    $dataAllCourses = new Courses;
                    $dataAllCourses = $dataAllCourses->select('courses.*');
                    $dataAllCourses = $dataAllCourses->leftJoin('topics', 'courses.id', '=', 'topics.courses_id');
                    $dataAllCourses = $dataAllCourses->whereIn('courses.id', array_unique($allCoursesIds));
                    $dataAllCourses = $dataAllCourses->where('courses.level_public', 1);
                    $dataAllCourses = $dataAllCourses->where('courses.status', '1');
                    $dataAllCourses = $dataAllCourses->where('topics.state', 'live');
                    $dataAllCourses = $dataAllCourses->where('topics.status', 1);
                    // $dataAllCourses = $dataAllCourses->orderBy($order_by, $order_direction)->paginate($per_page);
                    $dataAllCourses = $dataAllCourses->orderByRaw(
                    ...
                    $dataAllCourses = $dataAllCourses->distinct()->take($per_page)->get();
                    for($i=0; $i<count($dataAllCourses); $i++) {
                        if($dataAllCourses[$i]->latest_end_datetime >= date('Y-m-d')){
                            $dataAllCourses[$i]->latest = 1;
                        }else{
                    ...
                    return response()->json(array('data' => $dataAllCourses), 200);
                    ...
                    $data = $data->leftJoin('topics', 'courses.id', '=', 'topics.courses_id');

                    $data = $data->where('courses.level_public', 1);
                    $data = $data->where('courses.status', '1');
                    $data = $data->where('topics.state', 'live');
                    ...
                    $data = $data->distinct()->take($per_page)->get();
    ...
    public function courses($groupsKey, $id, _FunctionsController $oFunc)
    {
        ...
        for ($i=0; $i < count($data->agenda_live); $i++) { 
            $first_datetime = Carbon::parse($data->agenda_live[$i]->date)->format('Y-m-d 00:00:00');
            $last_datetime = Carbon::parse($data->agenda_live[$i]->date)->format('Y-m-d 23:59:59');
            $topic_start_datetime = $data->topics()->where('state', 'live')->whereBetween('live_start_datetime', array($first_datetime, $last_datetime))->where('status', 1)->orderBy('live_start_datetime', 'asc')->first();
            
            $topic_end_datetime = $data->topics()->where('state', 'live')->whereBetween('live_end_datetime', array($first_datetime, $last_datetime))->where('status', 1)->orderBy('live_end_datetime', 'desc')->first();
            ...
            $topic_live = $data->topics()->where('state', 'live')->where('live_start_datetime', '>=', $first_datetime)->where('live_end_datetime', '<=', $last_datetime)->where('status', 1)->first();
            ...
            $data->agenda_live[$i]->date = $oFunc->thai_date_fullmonth(strtotime($topic_end_datetime->live_end_datetime));
        }
        ...
        if(session()->get('_user')){
            ...
            $orders = Orders::where('members_id', $_user->id)->where('courses_id', $data->id)->where('order_status', 2);
            if ($orders->count()) {
    ...
    public function enroll2learning(Request $request, _SecurityController $_security, _FunctionsController $oFunc, SiteController $_site) {
        ...
        if(count($data)){
            ...
            $enroll->topics = $enroll->courses->topics()->whereNull('parent')->orderBy('order','asc')->get();
            for($a=0; $a<count($enroll->topics); $a++) {
                $enroll->topics[$a]->parent = Topics::where('parent', $enroll->topics[$a]->id)->orderBy('order','asc')->get();
            ...
            if($dataGroup->internal || $dataGroup->is_connect_regis != 1){
                ...
                try {
                    ...
                    if ($e->hasResponse()) {
                        ...
                        return response()->json(["error_msg" => "Server not available"], 500);
    ...
    public function enroll2topic($id, SiteController $_site){
        ...
        $null_enroll2topic = Enroll2Topic::where('enroll_id', $id)->whereNull('status')->get();

        if(count($null_enroll2topic)){

            $data = Enroll2Topic::where('enroll_id', $id)->whereNull('status')->orderBy('id', 'asc')->first();
    ...
    public function enroll2topic_live_skip($id, $topics_id){
        ...
        if($topics){
            ...
            // $enroll2topic = Enroll2TopicLive::where('enroll_id', $id)->where('topics_id', $topics_id)->count();
            // return response()->json($enroll2topic, 200);
            // if(!$enroll2topic){
            ...
            $is_success = $data->save();
            if ($is_success) {
                // $data = Enroll2TopicLive::where('enroll_id', $id)->where('topics_id', $topics_id)->first();
        ...
        for($i=0; $i<count($data); $i++) {
            ...
            for($a=0; $a<count($data[$i]->courses->topics); $a++) {
                ...
                for($x=0; $x<count($data[$i]->courses->topics[$a]->parent); $x++) {
                    ...
                    if ($data[$i]->courses->topics[$a]->parent[$x]->start_time == $data[$i]->courses->topics[$a]->parent[$x]->end_time) {
                        $flagIgnore = true;
                        break;
                    }
...
//            }
            ...
            if(($data->courses->learning == true) and ($data->exam->learning == true) and ($data->survey)){
                ...
                if (!isset($data->certificate_reference_number)) {
                    ...
                    if ($dataGroup->is_connect_regis == 1) {
                        ...
                        $data->certificate_reference_number = "SET".strtoupper($data->courses->code).$sessionNo.$data->members_id;
    ...
    public function createOrders(Request $request, _SecurityController $_security, _FunctionsController $oFunc, SiteController $_site)
    {
        ...
        if ($is_success) {
            ...
            $_site->logs('orders', '{"alert_msg":"success"}', 200, '', $_user['id'], json_encode($request->all(), JSON_UNESCAPED_UNICODE), $_user->groups_id, $_user->sub_groups_id, '', '');
        } else {
    ...
    public function discussion_list($groupsKey, $id, _SecurityController $_security, _FunctionsController $oFunc, SiteController $_site)
    {
        ...
        $data = $data->orWhere(function ($query) {
            $query->where('is_reject', '!=', 1)->where('is_public', 1)->where('status', 1);
        });
    ...
    public function discussion_instructors_list($groupsKey, $id, _SecurityController $_security, _FunctionsController $oFunc, SiteController $_site)
    {
        ...
        $data = new Discussions;
        $data = $data->whereNull('parent_id')->where('courses_id', $id)->where('groups_id', $groups['id'])->where('is_sent_instructor', 1)->where('is_reject', '!=', 1)->where('status', 1);
        $data = $data->orderBy('order', 'desc')->get();
        for($i=0; $i<count($data); $i++) {
            $data[$i]->members = $data[$i]->members()->select('first_name', 'last_name')->first();
    ...
    public function discussion_detail($id, _SecurityController $_security, _FunctionsController $oFunc, SiteController $_site)
    {
        ...
        $data = new Discussions;
        $data = $data->where('id', $id)->where('is_reject', '!=', 1)->where('status', 1)->first();

        $data->members = $data->members()->select('first_name', 'last_name')->first();
        $data->replies = Discussions::where('parent_id', $data->id)->where('is_reject', '!=', 1)->where('status', 1)->get();
        ...
        for($i=0; $i<count($data->replies); $i++) {
            ...
            $data->replies[$i]->replies = Discussions::where('parent_id', $data->replies[$i]->id)->where('is_reject', '!=', 1)->where('status', 1)->get();
    ...
    public function instructors_login($groups_key, $courses_id, Request $request, _SecurityController $_security, _FunctionsController $oFunc, SiteController $_site)
    {
        ...
        return response()->json(['groups_key' => $groups_key, 'course' => $course, 'code' => $request['code']], 500);
    }
