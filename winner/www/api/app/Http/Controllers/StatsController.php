...
class StatsController extends Controller
{
    ...
    public function info(Request $request, _RolesController $oRole)
    {
        ...
        if($request['courses_id']){
            $courses = new Courses();
            $courses = $courses->select('id', 'code', 'title');
    ...
    public function enroll(Request $request, _RolesController $oRole)
    {
        ...
        return response()->json(array('enroll' => $enroll, 'page_views' => $page_views, 'uip' => count($uip), 'fromDate' => $fromDate, 'toDate' => $toDate), 200);
    ...
    public function device(Request $request, _RolesController $oRole)
    {
        ...
        $all_windows = $data_all_windows->where('platform', 'like', '%Windows%')->count();
        ...
        $all_osx = $data_all_osx->where('platform', 'like', '%OS X%')->count();
        ...
        $all_linux = $data_all_linux->where('platform', 'like', '%linux%')->count();
        ...
        $all_ios = $data_all_ios->where('platform', 'like', '%iOS%')->count();
        ...
        $all_android = $data_all_android->where('platform', 'like', '%AndroidOS%')->count();
    ...
    public function learning(Request $request, _RolesController $oRole)
    {
        ...
        $data = new Enroll();
        $data->enroll = $data->whereHas('courses')->with([
            'courses.topics' => function($query) {
                $query->whereNotNull('parent')->orderBy('order','asc');
            },
            'enroll2topic.topics',
            'enroll2quiz' => function($query) {
                $query->where('type', 3)->whereNotNull('score')->orderBy('id','desc');
            },
            'enroll2quiz.quiz',
        ]);
        ...
        $dataEnroll = collect([]);
        $data->enroll->chunk(1000, function($enrolls) use (&$dataEnroll) {
            $dataEnroll = $dataEnroll->merge($enrolls);
        });

        // return response()->json(array_pluck($dataEnroll, 'id'), 500);

        $data->enroll = $dataEnroll;
        ...
        for($i=0; $i<count($data->enroll); $i++) {
            ...
            if (count($data->enroll[$i]->enroll2quiz) > 0) {
                $data->enroll[$i]->exam = $data->enroll[$i]->enroll2quiz[0];
            } else {
                $data->enroll[$i]->exam = null;
            }

            if($data->enroll[$i]->exam){
                // var_dump($data->enroll[$i]->exam->score);
                $data->exam += 1;
    ...
    public function courses(Request $request)
    {
        ...
        $data = new Courses;
        $data->courses = $data->with([
            'topics' => function($query) {
                $query->whereNull('parent')->orderBy('order','asc');
            },
            'topics.sub_topics' => function($query) {
                $query->orderBy('order','asc');
            }
        ])->find($request['courses_id']);
        // $data->topics = $data->courses->topics()->whereNull('parent')->orderBy('order','asc')->get();
        for($i=0; $i<count($data->courses->topics); $i++) {
            // $data->courses->topics[$i]->parent = Topics::where('parent', $data->courses->topics[$i]->id)->orderBy('order','asc')->get();
            for($a=0; $a<count($data->courses->topics[$i]->sub_topics); $a++) {
                // var_dump("$a");
                $data->courses->topics[$i]->sub_topics[$a]->duration = (strtotime($data->courses->topics[$i]->sub_topics[$a]->end_time) - strtotime('TODAY')) - (strtotime($data->courses->topics[$i]->sub_topics[$a]->start_time) - strtotime('TODAY'));
                $data->courses->topics[$i]->sub_topics[$a]->enroll2topic = $data->courses->topics[$i]->sub_topics[$a]->enroll2topic();
                $data->courses->topics[$i]->sub_topics[$a]->enroll2topic = $data->courses->topics[$i]->sub_topics[$a]->enroll2topic->with('enroll');
                $data->courses->topics[$i]->sub_topics[$a]->enroll2topic = $data->courses->topics[$i]->sub_topics[$a]->enroll2topic->whereHas('enroll', function($query) use ($groups_id, $sub_groups_id, $level_groups_id, $classrooms_id, $from_datetime, $to_datetime, $authSession, $authSessionGroups) {
                ...
                // $data->courses->topics[$i]->sub_topics[$a]->enroll2topic = $data->courses->topics[$i]->sub_topics[$a]->enroll2topic->get();

                $dataEnroll2topic = collect([]);
                $data->courses->topics[$i]->sub_topics[$a]->enroll2topic->chunk(1000, function($enroll2topic) use (&$dataEnroll2topic) {
                    $dataEnroll2topic = $dataEnroll2topic->merge($enroll2topic);
                });

                $data->courses->topics[$i]->sub_topics[$a]->enroll2topic = $dataEnroll2topic;

                $data->courses->topics[$i]->sub_topics[$a]->enroll2topic_count = $dataEnroll2topic->count();

                for($t=0; $t<$data->courses->topics[$i]->sub_topics[$a]->enroll2topic_count; $t++) {
                    if($data->courses->topics[$i]->sub_topics[$a]->enroll2topic[$t]->status){

                        $data->courses->topics[$i]->sub_topics[$a]->duration2enroll += $data->courses->topics[$i]->sub_topics[$a]->duration;
                        $data->courses->topics[$i]->sub_topics[$a]->duration2topic = $data->courses->topics[$i]->sub_topics[$a]->duration;
                        $data->courses->topics[$i]->sub_topics[$a]->duration2average = $data->courses->topics[$i]->sub_topics[$a]->duration2enroll / $data->courses->topics[$i]->sub_topics[$a]->enroll2topic_count;
                        $data->courses->topics[$i]->sub_topics[$a]->duration2percentage = ($data->courses->topics[$i]->sub_topics[$a]->duration2average / $data->courses->topics[$i]->sub_topics[$a]->duration2topic)*100;
                        $data->courses->topics[$i]->sub_topics[$a]->duration2gmdate = gmdate("H:i:s", $data->courses->topics[$i]->sub_topics[$a]->duration2average);

                        $data->courses->topics[$i]->sub_topics[$a]->enroll2topic_success += 1;

                    }else{

                        $data->courses->topics[$i]->sub_topics[$a]->duration2enroll += $data->courses->topics[$i]->sub_topics[$a]->enroll2topic[$t]->duration;
                        $data->courses->topics[$i]->sub_topics[$a]->duration2topic = $data->courses->topics[$i]->sub_topics[$a]->duration;
                        $data->courses->topics[$i]->sub_topics[$a]->duration2average = $data->courses->topics[$i]->sub_topics[$a]->duration2enroll / $data->courses->topics[$i]->sub_topics[$a]->enroll2topic_count;
                        $data->courses->topics[$i]->sub_topics[$a]->duration2percentage = ($data->courses->topics[$i]->sub_topics[$a]->duration2average / $data->courses->topics[$i]->sub_topics[$a]->duration2topic)*100;
                        $data->courses->topics[$i]->sub_topics[$a]->duration2gmdate = gmdate("H:i:s", $data->courses->topics[$i]->sub_topics[$a]->duration2average);

                        $data->courses->topics[$i]->sub_topics[$a]->enroll2topic_process += 1;

                    }
                }

                unset($data->courses->topics[$i]->sub_topics[$a]->enroll2topic);
            }
        }
    ...
    public function quiz(Request $request, _RolesController $oRole)
    {
        ...
        $dataPreTest = collect([]);
        $pre_test->data->chunk(1000, function($chunkPreTest) use (&$dataPreTest) {
            $dataPreTest = $dataPreTest->merge($chunkPreTest);
        });

        $pre_test->data = $dataPreTest;
        $pre_test->count = $pre_test->data->count();


        if($request['courses_id']){
            $pre_test->questions = Quiz::with([
                "questions" => function($query) {
                    $query->select('id', 'quiz_id', 'questions', 'type')->where('status', 1)->orderBy('order', 'asc');
                },
                "questions.answer" => function($query) {
                    $query->select('id', 'questions_id', 'answer', 'correct')->where('status', 1)->orderBy('order', 'asc');
                }
            ])->where('courses_id', $request['courses_id'])->where('type', 1)->where('status', 1)->first();
            ...
            $pre_test->questions = $pre_test->questions->questions;
        ...
        $dataPostTest = collect([]);
        $post_test->data->chunk(1000, function($chunkPostTest) use (&$dataPostTest) {
            $dataPostTest = $dataPostTest->merge($chunkPostTest);
        });

        $post_test->data = $dataPostTest;
        $post_test->count = $post_test->data->count();

        if($request['courses_id']){
            $post_test->questions = Quiz::with([
                "questions" => function($query) {
                    $query->select('id', 'quiz_id', 'questions', 'type')->where('status', 1)->orderBy('order', 'asc');
                },
                "questions.answer" => function($query) {
            ...
            ])->where('courses_id', $request['courses_id'])->where('type', 4)->where('status', 1)->first();
            ...
            $post_test->questions = $post_test->questions->questions;
        ...
        $dataCompare = collect([]);
        $compare->data->chunk(1000, function($chunkCompare) use (&$dataCompare) {
            $dataCompare = $dataCompare->merge($chunkCompare);
        });
        ...
        $exam->data = $exam->with(['enroll', 'quiz']);
        ...
        $dataExam = collect([]);
        $exam->data->chunk(1000, function($chunkExam) use (&$dataExam) {
            $dataExam = $dataExam->merge($chunkExam);
        });
        ...
        if($request['level_groups_id']){
            ...
            $exam->first_last_score = $exam->first_last_score->where('enroll_type', 4)->whereIn('enroll_type_id', array_pluck($level_groups, 'id'));
        }
        ...
        $dataFirstLastScore = collect([]);
        $exam->first_last_score->chunk(1000, function($chunkFirstLastScore) use (&$dataFirstLastScore) {
            $dataFirstLastScore = $dataFirstLastScore->merge($chunkFirstLastScore);
        });

        $exam->first_last_score = $dataFirstLastScore;
        ...
        if($request['courses_id']){
            ...
            ])->where('courses_id', $request['courses_id'])->where('type', 3)->where('status', 1)->first();
    ...
    public function export_enroll_bak(Request $request, _RolesController $oRole)
    {
        ini_set('memory_limit', '1024M');
        ini_set('max_execution_time', 3600);
    ...
    public function export_enroll(Request $request, _RolesController $oRole)
    {
        ...
        // Excel::create('ข้อมูลการเข้าเรียน '.date('Y-m-d H:i:s'), function($excel) use($courses_id, $groups_id, $sub_groups_id, $level_groups_id, $classrooms_id, $authSessionGroups, $authSession, $oRole, $from_datetime, $to_datetime) {
            // $excel->sheet('ข้อมูลการเข้าเรียน' , function($sheet) use ($courses_id, $groups_id, $sub_groups_id, $level_groups_id, $classrooms_id, $authSessionGroups, $authSession, $oRole, $from_datetime, $to_datetime) {
                ...
                $dataEnroll = collect([]);
                $data->chunk(1000, function($chunkEnroll) use (&$dataEnroll) {
                    $dataEnroll = $dataEnroll->merge($chunkEnroll);
                });
                ...
                for($i=0; $i<count($data); $i++) {
                    ...
                    if($data[$i]->enroll_type == 1){
                        ...
                        $data[$i]->classrooms = $data[$i]->classrooms->title;
                    } else if($data[$i]->enroll_type == 2){
                        $data[$i]->type = 'ClassRoom (Groups)';
                        $data[$i]->classrooms = ClassRooms::select('title')->find($data[$i]->enroll_type_id);
                        $data[$i]->classrooms = $data[$i]->classrooms->title;
                    } else if($data[$i]->enroll_type == 3){
                        $data[$i]->type = 'Targets';
                        $data[$i]->classrooms = '';
                    } else if($data[$i]->enroll_type == 4){
                        $data[$i]->type = 'Level Groups';
                        $data[$i]->classrooms = '';
                    }
                    else{
                    ...
                    // $data[$i]->pre_test = $data[$i]->enroll2quiz()->with('quiz')->where('type', 1)->orderBy('id', 'desc')->first();
                    ...
                    // $data[$i]->post_test = $data[$i]->enroll2quiz()->with('quiz')->where('type', 4)->orderBy('id', 'desc')->first();
                    ...
                    // $data[$i]->exam = $data[$i]->enroll2quiz()->with('quiz')->where('type', 3)->orderBy('id', 'desc')->first();
                    ...
                    // $sheet->row($i+2, array($data[$i]->id, $data[$i]->groups->title, $data[$i]->sub_groups_title, $data[$i]->members->first_name.' '.$data[$i]->members->last_name , $data[$i]->members->email, $data[$i]->courses->code, $data[$i]->courses->title, $data[$i]->type, $data[$i]->classrooms, $data[$i]->enroll_datetime, $data[$i]->last_datetime, $data[$i]->learning_status, $data[$i]->learning_percentage, $data[$i]->pre_test_status, $data[$i]->pre_test_score, $data[$i]->pre_test_percentage, $data[$i]->post_test_status, $data[$i]->post_test_score, $data[$i]->post_test_percentage, $data[$i]->exam_status, $data[$i]->exam_score, $data[$i]->exam_percentage, $data[$i]->course_status, $data[$i]->survey_status, $data[$i]->certificate, $data[$i]->certificate_reference_number, $data[$i]->certificate_datetime));
                ...
                // $debug_time = time() - $debug_start_time;
                // $debug_time .= "s";
                // return response()->json(["debug" => $data->count(), "debug_time" => $debug_time], 200);
            // });
        // })->download('xlsx');

    }

    public function export_quiz_bak(Request $request, _RolesController $oRole){
    ...
    public function export_quiz(Request $request, _RolesController $oRole,  _FunctionsController $oFunc){
        ...
        // Excel::create('ข้อมูลการทำข้อสอบ '.date('Y-m-d H:i:s'), function($excel) use($courses_id, $groups_id, $sub_groups_id, $level_groups_id, $classrooms_id, $authSessionGroups, $authSession, $oRole, $from_datetime, $to_datetime) {
            // $excel->sheet('ข้อมูลการทำข้อสอบ' , function($sheet) use ($courses_id, $groups_id, $sub_groups_id, $level_groups_id, $classrooms_id, $authSessionGroups, $authSession, $oRole, $from_datetime, $to_datetime) {
                ...
                $dataEnroll2Quiz = collect([]);
                $data->chunk($oFunc->cal_chunk($data->count()), function($enroll2Quiz) use (&$dataEnroll2Quiz) {
                    $dataEnroll2Quiz = $dataEnroll2Quiz->merge($enroll2Quiz);
                });

                $data = $dataEnroll2Quiz;
                $list = [];
                ...
                for($i=0; $i<count($data); $i++) {
                    ...
                    if($data[$i]->enroll->sub_groups_id){
                        // $data[$i]->enroll->sub_groups = $data[$i]->enroll->sub_groups()->first();
                        $data[$i]->enroll->sub_groups_title = $data[$i]->enroll->sub_groups->title;
                    }else{
                        $data[$i]->enroll->sub_groups_title = '';
                    }
                    // $data[$i]->enroll->members = $data[$i]->enroll->members()->first();
                    // $data[$i]->enroll->courses = $data[$i]->enroll->courses()->first();
                    // $data[$i]->quiz = $data[$i]->quiz()->first();
                    ...
                    // $sheet->row($i+2, array($data[$i]->enroll->id, $data[$i]->enroll->groups->title, $data[$i]->enroll->sub_groups_title, $data[$i]->enroll->members->first_name.' '.$data[$i]->enroll->members->last_name, $data[$i]->enroll->members->email, $data[$i]->enroll->courses->code, $data[$i]->enroll->courses->title, $data[$i]->quiz->title, $data[$i]->quiz->quiz_type, $data[$i]->score, $data[$i]->count, $data[$i]->percentage, $data[$i]->quiz_status, $data[$i]->datetime));

                    $list[] = [
                        'Enroll ID.' => $data[$i]->enroll->id,
                        'Groups' => $data[$i]->enroll->groups->title,
                        'Sub Groups' => $data[$i]->enroll->sub_groups_title,
                        'ชื่อ - สกุล' => $data[$i]->enroll->members->first_name.' '.$data[$i]->enroll->members->last_name,
                        'อีเมล์' => $data[$i]->enroll->members->email,
                        'รหัสหลักสูตร' => $data[$i]->enroll->courses->code,
                        'ชื่อหลักสูตร' => $data[$i]->enroll->courses->title,
                        'ชุดคำถาม' => $data[$i]->quiz->title,
                        'ประเภท' => $data[$i]->quiz->quiz_type,
                        'คะแนน' => $data[$i]->score,
                        'คะแนนเต็ม' => $data[$i]->count,
                        'คิดเป็น​ (%)' => $data[$i]->percentage,
                        'สถานะ' => $data[$i]->quiz_status,
                        'วันเวลาทำข้อสอบ' => $data[$i]->datetime,
                    ];
                }
                ...
                $list = collect($list);
                $sheets = new SheetCollection([
                    'ข้อมูลการทำข้อสอบ' => $list
                ]);

                $filename = '_ข้อมูลการทำข้อสอบ '.date('Y-m-d H:i:s').'.xlsx';
                return FastExcel::data($sheets)->headerStyle($headerStyle)->dataStyle($dataStyle)->download($filename);
            // });
        // })->download('xlsx');
    ...
    public function export_course_bak(Request $request, _RolesController $oRole){
    ...
    public function export_course(Request $request, _RolesController $oRole){
        ...
        // Excel::create('ข้อมูลภาพรวมหลักสูตร '.date('Y-m-d H:i:s'), function($excel) use($courses_id, $groups_id, $sub_groups_id, $level_groups_id, $classrooms_id, $authSessionGroups, $authSession, $oRole, $from_datetime, $to_datetime) {
            // $excel->sheet('ข้อมูลภาพรวมหลักสูตร' , function($sheet) use ($courses_id, $groups_id, $sub_groups_id, $level_groups_id, $classrooms_id, $authSessionGroups, $authSession, $oRole, $from_datetime, $to_datetime) {
                ...
                $dataCourses = collect([]);
                $data->chunk(100, function($courses) use (&$dataCourses) {
                    $dataCourses = $dataCourses->merge($courses);
                });
                ...
                for($i=0; $i<count($data); $i++) {
                    ...
                    // $data[$i]->topics = $data[$i]->topics()->whereNotNull('parent')->orderBy('order','asc')->get();
                    for($a=0; $a<count($data[$i]->topics); $a++) {
                        $data[$i]->topics[$a]->duration = (strtotime($data[$i]->topics[$a]->end_time) - $timeToday) - (strtotime($data[$i]->topics[$a]->start_time) - $timeToday);
                        $data[$i]->full_duration += $data[$i]->topics[$a]->duration;
                    }

                    $data[$i]->enroll = $data[$i]->enroll()->with([
                        'enroll2topic',
                        'enroll2topic.topics',
                        'latest_exam_with_score'
                    ]);
                    ...
                    if($level_groups_id){
                        ...
                        $data[$i]->enroll = $data[$i]->enroll->where('enroll_type', 3)->orWhere(function($query) use ($level_groups, $i) {
                            $query->where('enroll_type', 4)->whereIn('enroll_type_id', array_pluck($level_groups, 'id'));
                        });
                    }
                    ...
                    // $data[$i]->enroll = $data[$i]->enroll->whereBetween('enroll_datetime',array($from_datetime, $to_datetime))->get();
                    $data[$i]->enroll = $data[$i]->enroll->whereBetween('enroll_datetime',array($from_datetime, $to_datetime));

                    $dataEnrolls = collect([]);
                    $data[$i]->enroll->chunk(1000, function($enrolls) use (&$dataEnrolls) {
                        $dataEnrolls = $dataEnrolls->merge($enrolls);
                    });

                    $data[$i]->enroll = $dataEnrolls;
                    ...
                    for($e=0; $e<count($data[$i]->enroll); $e++) {
                        ...
                        for($q=0; $q<count($data[$i]->enroll[$e]->enroll2topic); $q++) {
                            if($data[$i]->enroll[$e]->enroll2topic[$q]->status == 1){
                                // $data[$i]->enroll[$e]->enroll2topic[$q]->topics = $data[$i]->enroll[$e]->enroll2topic[$q]->topics()->first();
                                $data[$i]->enroll[$e]->duration += (strtotime($data[$i]->enroll[$e]->enroll2topic[$q]->topics->end_time) - $timeToday) - (strtotime($data[$i]->enroll[$e]->enroll2topic[$q]->topics->start_time) - $timeToday); 
                            }else{
                                $data[$i]->enroll[$e]->duration += $data[$i]->enroll[$e]->enroll2topic[$q]->duration;
                            }
                        }

                        if ($data[$i]->enroll[$e]->duration == 0) {
                            $data[$i]->not_learning += 1;
                        } else {
                            $data[$i]->enroll[$e]->progress = number_format(($data[$i]->enroll[$e]->duration / $data[$i]->full_duration)*100);
                            if($data[$i]->enroll[$e]->progress >= $data[$i]->percentage){
                        ...
                        // $data[$i]->enroll[$e]->exam = $data[$i]->enroll[$e]->enroll2quiz()->where('type', 3)->whereNotNull('score')->orderBy('id','desc')->first();
                        $data[$i]->enroll[$e]->exam = $data[$i]->enroll[$e]->latest_exam_with_score;
                        if($data[$i]->enroll[$e]->exam){
                            $data[$i]->exam += 1;
                            $data[$i]->enroll[$e]->exam->percentage = number_format(($data[$i]->enroll[$e]->exam->score/$data[$i]->enroll[$e]->exam->count)*100);
                            // $data[$i]->enroll[$e]->exam->quiz = $data[$i]->enroll[$e]->exam->quiz()->first();
                    ...
                    $enrollCount = $data[$i]->enroll->count();
                    if($enrollCount){
                        $data[$i]->enroll_all = $enrollCount;
                    }else{
                        $data[$i]->enroll_all = null;
                    }
                    ...
                    $dataCompares = collect([]);
                    $data[$i]->compare->data->chunk(1000, function($compares) use (&$dataCompares) {
                        $dataCompares = $dataCompares->merge($compares);
                    });

                    $data[$i]->compare->data = $dataCompares;
                    ...
                    //Exam
                    $data[$i]->exam = new Enroll2Quiz();
                    $data[$i]->exam->data = $data[$i]->exam->with(['enroll', 'quiz']);
                    $data[$i]->exam->data = $data[$i]->exam->data->where('type', 3)->whereNotNull('score');
                    $data[$i]->exam->data = $data[$i]->exam->data->whereHas('enroll', function($query) use ($courses_id, $groups_id, $sub_groups_id, $level_groups_id, $classrooms_id, $authSessionGroups, $authSession, $from_datetime, $to_datetime) {
                    ...
                    $data[$i]->exam->first_last_score = new Enroll();
                    $data[$i]->exam->first_last_score = $data[$i]->exam->first_last_score->with(['first_exam_with_score', 'latest_exam_with_score']);
                    ...
                    for($c=0; $c<count($data[$i]->exam->first_last_score); $c++) {
                        ...
                        $data[$i]->exam->first_last_score[$c]->asc = $data[$i]->exam->first_last_score[$c]->first_exam_with_score;
                        $data[$i]->exam->first_last_score[$c]->desc = $data[$i]->exam->first_last_score[$c]->latest_exam_with_score;
                    ...
                    $list[] = [
                        'รหัสหลักสูตร' => $data[$i]->code,
                        'ชื่อหลักสูตร' => $data[$i]->title,
                        'จำนวนคนที่ลงทะเบียนเข้าเรียนหลักสูตรทั้งหมด (คน)' => $data[$i]->enroll_all,
                        'ลงทะเบียน (คน)' => $data[$i]->not_learning,
                        'กำลังเรียน (คน)' => $data[$i]->learning_not_pass,
                        'ยังไม่สอบ (คน)' => $data[$i]->learning_pass_not_exam,
                        'สอบไม่ผ่าน (คน)' => $data[$i]->exam_not_pass,
                        'จำนวนคนที่ยังไม่ผ่านหลักสูตร (คน)' => $data[$i]->not_pass,
                        'ผ่าน (คน)' => $data[$i]->exam_pass,
                        'ไม่พิมพ์วุฒิบัตร (คน)' => $data[$i]->not_certificate,
                        'พิมพ์วุฒิบัตร (คน)' => $data[$i]->certificate,
                        'ทำแบบทดสอบก่อนเรียน (ครั้ง)' => $data[$i]->pre_test->count,
                        'ทำแบบทดสอบหลังเรียน (ครั้ง)' => $data[$i]->post_test->count,
                        'Post Test ได้คะแนนมากกว่า Pre Test (ครั้ง)' => $data[$i]->compare->over,
                        'Post Test ได้คะแนนมากกว่า Pre Test (%)' => $data[$i]->compare->over_percentage,
                        'Post Test ได้คะแนนน้อยกว่าหรือเท่ากับ Pre Test (ครั้ง)' => $data[$i]->compare->under,
                        'Post Test ได้คะแนนน้อยกว่าหรือเท่ากับ Pre Test (%)' => $data[$i]->compare->under_percentage,
                        'จำนวนการทำแบบทดสอบ (ครั้ง)' => $data[$i]->exam->count,
                        'ทำแบบทดสอบผ่าน (ครั้ง)' => $data[$i]->exam->exam_pass,
                        'แบบทดสอบไม่ผ่าน (ครั้ง)' => $data[$i]->exam->exam_not_pass,
                        'ทำแบบทดสอบเฉลี่ยกี่ครั้งถึงผ่าน (ครั้ง)' => $data[$i]->exam->average_to_quiz,
                        'คะแนนครั้งแรกเฉลี่ย' => $data[$i]->exam->first_score_average,
                        'คะแนนครั้งล่าสุดเฉลี่ย' => $data[$i]->exam->last_score_average,
                        'คะแนนสูงสุดเฉลี่ย' => $data[$i]->exam->max,
                        'คะแนนต่ำสุดเฉลี่ย' => $data[$i]->exam->min,
                    ];
                }
                ...
                $list = collect($list);
                $sheets = new SheetCollection([
                    'ข้อมูลภาพรวมหลักสูตร' => $list
                ]);

                $filename = '_ข้อมูลภาพรวมหลักสูตร '.date('Y-m-d H:i:s').'.xlsx';
                return FastExcel::data($sheets)->headerStyle($headerStyle)->dataStyle($dataStyle)->download($filename);
    ...
    public function export_questions_bak(Request $request, _RolesController $oRole){
    ...
    public function export_questions(Request $request, _RolesController $oRole){
        ...
        Excel::create("_".$data->title.' ('.$data->courses->code.' '.$data->courses->title.') '.date('Y-m-d H:i:s'), function($excel) use($quiz_id, $courses_id, $groups_id, $sub_groups_id, $level_groups_id, $classrooms_id, $authSessionGroups, $authSession, $oRole, $from_datetime, $to_datetime) {
            $excel->sheet('ข้อมูลการทำข้อสอบ' , function($sheet) use ($quiz_id, $courses_id, $groups_id, $sub_groups_id, $level_groups_id, $classrooms_id, $authSessionGroups, $authSession, $oRole, $from_datetime, $to_datetime) {
                $sheet->row(1, array('ข้อคำถาม', 'คำถาม', 'ข้อคำตอบ', 'คำตอบ', 'ตอบ', 'เฉลยข้อที่ถูก'));
