...
class ClassRoomsController extends Controller
{
    ...
    public function index(Request $request, _RolesController $oRole)
    {
        ...
        for($i=0; $i<count($data); $i++) {
            ...
            $data[$i]->sub_groups = $data[$i]->sub_groups()->get();
            $data[$i]->level_groups = $data[$i]->level_groups()->get();
            $data[$i]->members = $data[$i]->members()->get();
            $data[$i]->courses = $data[$i]->courses()->get();
            $data[$i]->groups = $data[$i]->groups()->first();
            if(($data[$i]->start_datetime <= date('Y-m-d H:i:s')) and ($data[$i]->end_datetime >= date('Y-m-d H:i:s'))){
                $data[$i]->session = 'Online';
            }else if(($data[$i]->start_datetime >= date('Y-m-d H:i:s'))){
                $data[$i]->session = 'Coming...';
            }else{
                $data[$i]->session = 'End';
            }
        }
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($input, [
            ...
            'start_datetime' => 'required|date|date_format:Y-m-d H:i:s|before:end_datetime',
            'end_datetime' => 'required|date|date_format:Y-m-d H:i:s|after:start_datetime',
        ]);
        ...
        if (isset($input['classroom2sub_group'])) {
            $classroom2sub_group = $input['classroom2sub_group'];
            $data->sub_groups()->sync($classroom2sub_group);
        }

        if (isset($input['classroom2level_group'])) {
            $classroom2level_group = $input['classroom2level_group'];
            $data->level_groups()->sync($classroom2level_group);
        }

        if (isset($input['classroom2member'])) {
            $classroom2member = $input['classroom2member'];
            $data->members()->sync($classroom2member);
        }

        if (isset($input['classroom2course'])) {
            $classroom2course = $input['classroom2course'];
            $data->courses()->sync($classroom2course);
        }

        if ($is_success) {
            $message = "The classrooms has been created.";
        } else {
            $message = "Failed to create the classrooms.";
        }
    ...
    public function show($id, _RolesController $oRole)
    {
        ...
        if (!$oRole->haveAccess($id, "classrooms")) {
        ...
        if ($authSession->super_users) {
            ...
            $data->level_groups = $data->level_groups()->where('level_groups.admins_id', $authSession->id)->get();
            $data->courses = $data->courses()->where('admins_id', $authSession->id)->get();
        } else if (!$oRole->isSuper()) {
            $data->sub_groups = $data->sub_groups()->whereIn('groups_id', array_pluck($authSessionGroups, 'id'))->get();
            $data->level_groups = $data->level_groups()->whereIn('level_groups.groups_id', array_pluck($authSessionGroups, 'id'))->get();
            $data->courses = $data->courses()->whereHas('groups', function($query) use ($authSessionGroups) {
        ...
        switch ($data->groups->field_approval) {
            case 'email'         : $data->groups->field_approval_text = "อีเมล์"; break;
            case 'full_name'     : $data->groups->field_approval_text = "ชื่อ - นามสกุล"; break;
            case 'id_card'       : $data->groups->field_approval_text = "เลขบัตรประจำตัวประชาชน"; break;
            case 'license_id'    : $data->groups->field_approval_text = "เลขที่ใบอนุญาต"; break;
            case 'occupation_id' : $data->groups->field_approval_text = $data->groups->meaning_of_occupation_id; break;
        }
    ...
    public function update($id, Request $request, _RolesController $oRole)
    {
        ...
        if (is_array($input['classroom2sub_group'])) {
            $classroom2sub_group = $input['classroom2sub_group'];

            if ($authSession->super_users) {
                $dataSubGroups = $data->sub_groups()->where('sub_groups.id', $authSession->sub_groups_id)->get();
                $data->sub_groupsBySubGroups(array_pluck($dataSubGroups, 'id'))->sync($classroom2sub_group);
            } else if (!$oRole->isSuper()) {
        ...
        if (is_array($input['classroom2level_group'])) {
            $classroom2level_group = $input['classroom2level_group'];

            if ($authSession->super_users) {
                $dataLevelGroups = $data->level_groups()->where('level_groups.admins_id', $authSession->id)->get();
                $data->level_groupsByLevelGroups(array_pluck($dataLevelGroups->toArray() + $authSession->admin2level_group->toArray(), 'id'))->sync($classroom2level_group);
            } else if (!$oRole->isSuper()) {
                $dataLevelGroups = $data->level_groups()->whereIn('level_groups.groups_id', array_pluck($authSessionGroups, 'id'))->get();
                $data->level_groupsByLevelGroups(array_pluck($dataLevelGroups, 'id'))->sync($classroom2level_group);
            } else {
                $data->level_groups()->sync($classroom2level_group);
            }
        }

        // if (is_array($input['classroom2member'])) {
        //     $classroom2member = $input['classroom2member'];
        //     $data->members()->sync($classroom2member);
        // }

        if (is_array($input['classroom2course'])) {
            $classroom2course = $input['classroom2course'];

            if ($authSession->super_users) {
                $dataCourses = $data->courses()->where('admins_id', $authSession->id)->get();
                $data->coursesByCourses(array_pluck($dataCourses, 'id'))->sync($classroom2course);
            } else if (!$oRole->isSuper()) {
                $dataCourses = $data->courses()->whereHas('groups', function($query) use ($authSessionGroups) {
                    $query->whereIn('groups_id', array_pluck($authSessionGroups, 'id'));
                })->get();
        ...
        if ($is_success) {
            $message = "The classrooms has been updated.";
        } else {
            $message = "Failed to update the classrooms.";
        }
    ...
    public function destroy($id, _RolesController $oRole)
    {
        ...
        if ($is_success) {
            $message = "The classrooms has been deleted.";
        } else {
            $message = "Failed to delete the classrooms.";
        }
        return response()->json(array('is_error' => !$is_success, 'message'=>$message), 200);
    }
    ...
    public function updateStatus($id, Request $request)
    {
        ...
        if ($is_success) {
            ...
            $message = "Failed to update the classroom.";
        }
    ...
    public function orders(Request $request)
    {
        ...
        for($i=0; $i<count($input); $i++) {
            $data[$i] = ClassRooms::find($input[$i]['id']);
            $data[$i]->fill($input[$i]);
            $data[$i]->save();
        }
    }
    ...
    public function sort(Request $request)
    {
        ...
        $entity = ClassRooms::find($request['id']);

        if (is_numeric($request['positionEntityId'])) {
            $positionEntity = ClassRooms::find($request['positionEntityId']);
            ...
            $data = ClassRooms::where('order', '=', $request['order'])->first();
            ...
            if ($data) {
                ...
                $last = ClassRooms::orderBy('order', 'desc')->first();
                if ($request['order'] > $last->order) {
                    $entity->moveAfter($last);
                } else {
                    $next = ClassRooms::where('order', '>', $request['order'])->min('id');
                    ClassRooms::find($next)->decrement('order');
                    $entity->moveBefore(ClassRooms::find($next));
                }
            }
        } else {
        ...
        $message = "The classrooms has been sorted.";
    ...
    private function updateMembersSSO($dataClassRoom, $dataGroups, $dataLevelGroups, $dataMembers, $dataFromExcel, $active, $activeRemark, $approved_type = null)
    {
        $authSession = Auth::user();
        ...
        $dataMembers->classrooms()->syncWithoutDetaching([$dataClassRoom->id]);

        return $dataMembers;
    }

    private function updateMembers($dataClassRoom, $dataGroups, $dataLevelGroups, $dataMembers, $dataFromExcel, $active, $activeRemark, $approved_type = null, $allowUpdateEmail = true)
    {
        $authSession = Auth::user();
    ...
    private function updateMembersPreApproved($dataClassRoom, $dataGroups, $dataLevelGroups, $dataMembersPreApproved, $dataFromExcel)
    {
        ...
        $dataMembersPreApproved->classrooms()->syncWithoutDetaching([$dataClassRoom->id]);
    ...
    private function createMembersPreApproved($dataClassRoom, $dataGroups, $dataLevelGroups, $dataFromExcel)
    {
    ...
    private function createMembers($dataClassRoom, $dataGroups, $dataLevelGroups, $dataFromExcel)
    {
        $oFunc = new _FunctionsController;
    ...
    private function readMembersPreApprovedExcel($fileExcel, $countHeader, $dataClassRoom, $dataGroups)
    {
        $oFunc = new _FunctionsController;
        $authSession = Auth::user();
        $dataInsert = array();
                    ...
                    if ($dataExist) {
                        if ($dataExist->sub_groups_id == $arrDetail[9]) {
                            if ($dataExist->active == 1) {
                                $dataClassRoomExist = $dataExist->classrooms()->where('classrooms_id', $dataClassRoom->id)->first();
                                if (!$dataClassRoomExist) {

                                    if (empty($dataExist->last_login)) {
                                        $dataUpdate = $this->updateMembers($dataClassRoom, $dataGroups, $dataLevelGroups, $dataExist, $arrDetail, 1, 0);
                                        $dataUpdated[] = ["row" => $row] + $dataUpdate->toArray() + ["message" => "This member has been updated. (Added member to classroom)"];
                                        ...
                                        $dataExist->classrooms()->syncWithoutDetaching([$dataClassRoom->id]);
                                        $dataUpdated[] = ["row" => $row] + $dataExist->toArray() + ["message" => "This member already exists. (Added member to classroom)"];
                                        $dataMember = $dataExist;
                                    }

                                    /* BEGIN E-MAIL FUNCTION */
                                    // Notify Mail (Confirm Register ClassRoom *use old member)
                                    ...
                                    Mail::send('classrooms-register-oldmembers-mail', $dataMail, function($mail) use ($dataMail) {
                                        ...
                                        $mail->to($dataMail['dataMembers']['email'], $receiverName)->subject('แจ้งยืนยันการลงทะเบียนคลาส '.$dataMail['dataClassRoom']['title']);
                                    ...
                                    if (empty($dataExist->last_login)) {
                                        ...
                                        if (!$dataExist->level_groups()->where('level_groups.id', $arrDetail[11])->first()) {
                                            ...
                                            $dataUpdated[] = ["row" => $row] + $dataExist->toArray() + ["message" => "This member already exists in this classroom."];
                                        }
                                    }
                                }
                            } else {
                                $dataUpdate = $this->updateMembers($dataClassRoom, $dataGroups, $dataLevelGroups, $dataExist, $arrDetail, 1, 3, 1);
                                ...
                                Mail::send('classrooms-reactivated-mail', $dataMail, function($mail) use ($dataMail) {
                                    if ($dataMail['dataMembers']['is_foreign'] != 1) {
                                        $receiverName = $dataMail['dataMembers']['first_name']." ".$dataMail['dataMembers']['last_name'];
                                    } else {
                                        $receiverName = $dataMail['dataMembers']['first_name_en']." ".$dataMail['dataMembers']['last_name_en'];
                            ...
                            if ($countNewEmailExist > 0) {
                                ...
                                if (empty($dataExist->last_login)) {
                                    $dataUpdate = $this->updateMembers($dataClassRoom, $dataGroups, $dataLevelGroups, $dataExist, $arrDetail, 1, 2, null, false);
                                    $dataMember = $dataUpdate;
                                } else {
                                    $dataExist->active = 1;
                                ...
                                Mail::send('classrooms-change-subgroups-alert-mail', $dataMail, function($mail) use ($dataMail) {
                        ...
                        if ($dataPreApprovedExist) {
                            $dataUpdate = $this->updateMembersPreApproved($dataClassRoom, $dataGroups, $dataLevelGroups, $dataPreApprovedExist, $arrDetail);
                            if ($dataUpdate) {
                                $dataUpdated[] = ["row" => $row] + $dataUpdate->toArray() + ["message" => "This member has been updated."];
                            }
                        } else {
                            $dataCreate = $this->createMembersPreApproved($dataClassRoom, $dataGroups, $dataLevelGroups, $arrDetail);
                            if ($dataCreate) {
                    ...
                    if ($dataExist) {
                        if ($dataExist->sub_groups_id == $arrDetail[9]) {
                            ...
                            if ($dataGroups->internal == 1) {
                                ...
                                $dataUpdate = $this->updateMembersSSO($dataClassRoom, $dataGroups, $dataLevelGroups, $dataExist, $arrDetail, 1, 1);
                                $dataUpdated[] = ["row" => $row] + $dataUpdate->toArray() + ["message" => "This member has been updated."];
                            }
                        } else {
                            $countNewEmailExist = Members::where("email", $arrDetail[4])->where('groups_id', $dataGroups->id)->where('id', '!=', $dataExist->id)->count();
    ...
    private function readMembersExcel($fileExcel, $countHeader, $dataClassRoom, $dataGroups)
    {
        $oFunc = new _FunctionsController;
        $authSession = Auth::user();
        $dataInsert = array();
            ...
            if (!is_null($dataFormValidations['errorMessage'])) {
                ...
                if ($dataGroups->id == 3) {
                    ...
                    if ($dataExist) {
                        ...
                        // Add New Member
                        $dataCreate = $this->createMembers($dataClassRoom, $dataGroups, $dataLevelGroups, $arrDetail);
                        if ($dataCreate) {
                            $dataInserted[] = ["row" => $row] + $dataCreate->toArray();
                            ...
                            /* BEGIN E-MAIL FUNCTION */
                            // Notify Mail (Confirm Register ClassRoom *use new member)
                            ...
                            Mail::send('classrooms-register-newmembers-mail', $dataMail, function($mail) use ($dataMail) {
                                if ($dataMail['dataMembers']['is_foreign'] != 1) {
                                    $receiverName = $dataMail['dataMembers']['first_name']." ".$dataMail['dataMembers']['last_name'];
                                } else {
                                    $receiverName = $dataMail['dataMembers']['first_name_en']." ".$dataMail['dataMembers']['last_name_en'];
    ...
    public function importPreApprovedMembers(Request $request, _FunctionsController $oFunc, $id)
    {
        ...
        $dataClassRoom = ClassRooms::find($id);

        if (!$dataClassRoom) {
            return response()->json(array('message' => str_replace("{{_RESOURCE_}}", "'ClassRoom'", config('constants._errorMessage._404'))), 404);
        }
        ...
        // $dataGroups = $dataClassRoom->sub_groups()->with('groups')->first()->groups;

        // $dataExcel = $this->readMembersPreApprovedExcel($fileExcel, 1, $dataClassRoom, $dataGroups);
    ...
    public function importMembers(Request $request, _FunctionsController $oFunc, $id)
    {
        ...
        // $dataExcel = $this->readMembersExcel($fileExcel, 1, $dataClassRoom, $dataGroups);
    ...
    public function detachMembers($id, Request $request)
    {
        ...
        if ($is_success) {
            $message = "The members has been detached from this class room.";
        } else {
            $message = "Failed to detach the members from this class room.";
        }
        return response()->json(array('is_error' => !$is_success, 'message' => $message), 200);
    }
