...
class AnswerController extends Controller
{
    ...
    {
        //
        $data = Answer::orderBy($request['order_by'],$request['order_direction'])->paginate($request['per_page']);
    ...
    public function sort(Request $request)
    {
        ...
        $entity = Answer::find($request['id']);

        if (is_numeric($request['positionEntityId'])) {
            $positionEntity = Answer::find($request['positionEntityId']);
            ...
            $data = Answer::where('order', '=', $request['order'])->first();
            ...
            if ($data) {
                ...
                $last = Answer::orderBy('order', 'desc')->first();
                if ($request['order'] > $last->order) {
                    $entity->moveAfter($last);
                } else {
                    $next = Answer::where('order', '>', $request['order'])->min('id');
                    Answer::find($next)->decrement('order');
                    $entity->moveBefore(Answer::find($next));
                }
            }
        } else {
        ...
        $message = "The answer has been sorted.";
