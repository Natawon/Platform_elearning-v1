...
angular.module('newApp')
    .controller('slidesUploadCtrlTopics', ['$scope', 'FileUploader', 'settingsFactory', function($scope, FileUploader, settingsFactory) {

        var img_uploader = $scope.img_uploader = new FileUploader({
            url: settingsFactory.getUpload("upload_slides_picture")
        });
        ...
        img_uploader.onSuccessItem = function(fileItem, response, status, headers) {
            ...
            if ($scope.mode == 'Create') {
                $scope.sync_by_topics.picture = response.file_name;
            } else {
                $scope.slides_topics_data.picture = response.file_name;
            }
        };
