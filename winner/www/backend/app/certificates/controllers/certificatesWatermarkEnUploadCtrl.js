...
angular.module('newApp')
    .controller('certificatesWatermarkEnUploadCtrl', ['$scope', 'FileUploader', 'settingsFactory', function($scope, FileUploader, settingsFactory) {

        var img_uploader = $scope.img_uploader = new FileUploader({
            url: settingsFactory.getUpload("upload_certificates_watermark_en")
        });
        ...
        img_uploader.onSuccessItem = function(fileItem, response, status, headers) {
            ...
            $scope.certificates_data.watermark_en = response.file_name;
        };
