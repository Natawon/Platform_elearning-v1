...
angular.module('newApp')
    .controller('certificatesLogoUploadCtrl', ['$scope', 'FileUploader', 'settingsFactory', function($scope, FileUploader, settingsFactory) {

        var img_uploader = $scope.img_uploader = new FileUploader({
            url: settingsFactory.getUpload("upload_certificates_logo")
        });
        ...
        img_uploader.onSuccessItem = function(fileItem, response, status, headers) {
            ...
            $scope.certificates_data.logo = response.file_name;
        };
