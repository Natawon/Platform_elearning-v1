...
angular.module('newApp')
    .controller('certificatesSignatureUploadCtrl', ['$scope', 'FileUploader', 'settingsFactory', function($scope, FileUploader, settingsFactory) {

        var img_uploader = $scope.img_uploader = new FileUploader({
            url: settingsFactory.getUpload("upload_certificates_signature")
        });
        ...
        img_uploader.onSuccessItem = function(fileItem, response, status, headers) {
            ...
            $scope.certificates_data.signature = response.file_name;
        };
