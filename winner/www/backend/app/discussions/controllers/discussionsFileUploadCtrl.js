...
angular.module('newApp')
    .controller('discussionsFilelUploadCtrl', ['$scope', 'FileUploader', 'settingsFactory', function($scope, FileUploader, settingsFactory) {

        var img_uploader = $scope.img_uploader = new FileUploader({
            url: settingsFactory.getUpload("upload_discussions_file")
        });
        ...
        img_uploader.onSuccessItem = function(fileItem, response, status, headers) {
            ...
            if ($scope.type_discussions_file === "send") {
                $scope.discussions_data_send.file = response.file_name;
            } else {
                $scope.discussions_data.file = response.file_name;
            }
        };
