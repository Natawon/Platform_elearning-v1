angular.module('newApp').factory('pluginsService', [function () {
    ...
    function colorPicker() {
        if ($('.color-picker').length && $.fn.spectrum) {
            $('.color-picker').each(function () {
                ...
                $(this).spectrum({
                    preferredFormat: $(this).data('format') ? $(this).data('format') : "hex",
                    ...
                    palette: $(this).data('palette') ? $(this).data('palette') : $(this).data('use-default-palette') ? default_palette : [[current_palette]]
                });
                $(this).show();
            });
        }
    ...
    function popover() {
        ...
        if ($('[rel="popover_dark"]').length && $.fn.popover) {
            $('[rel="popover_dark"]').each(function () {
                $(this).popover({
        ...
        if ($('[rel="popover_wide"]').length && $.fn.popover) {
            $('[rel="popover_wide"]').each(function () {
                $(this).popover({
                    template: '<div class="popover popover-wide"><div class="arrow"></div><h3 class="popover-title popover-title"></h3><div class="popover-content popover-content"></div></div>',
                    trigger: $(this).data('trigger') ? $(this).data('trigger') : "hover",
                    html: true
                });
            });
    ...
    function inputTags(typeaheadSource) {
        $('.select-tags').each(function () {
            $(this).tagsinput({
                tagClass: $(this).data('tag-class') ? $(this).data('tag-class') : 'label label-primary',
            });

            // console.log(typeaheadSource);

            // if (typeaheadSource) {
            //     if (angular.isArray(typeaheadSource)) {
            //         var dataList = new Bloodhound({
            //             datumTokenizer: Bloodhound.tokenizers.obj.whitespace('name'),
            //             queryTokenizer: Bloodhound.tokenizers.whitespace,
            //             local: $.map(typeaheadSource, function (name) {
            //                 return {
            //                     name: name
            //                 };
            //             })
    ...
    function animateNumber() {
        setTimeout(function() {
            $('.countup').each(function () {
                ...
                to = $(this).val() ? $(this).val() : $(this).data("to") ? $(this).data("to") : 100;
    ...
    return {
        ...
        popoverWithOptions: function() {
            return popoverWithOptions();
        },
        ...
        inputTags: function(typeaheadSource) {
            return inputTags(typeaheadSource);
        },
        sliderIOS: function() {
            return sliderIOS();
        }
