    ...
    function initializeMasterCheckboxEventHandler(on) {
        ...
        if (on) {
            ...
            $('#masterCheck', tableElement).on('click', function () {
                ...
                //
                // toggles all checkboxes but does not trigger click courses.  Because there's
                // no click event, there's no changed courses on the checkboxes.  We need the
                // changed courses so that we can keep track of the checked checkboxes.
                if (this.checked) {
