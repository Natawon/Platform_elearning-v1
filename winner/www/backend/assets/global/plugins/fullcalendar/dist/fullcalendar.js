...
// different contexts as identical when binding/unbinding courses.
...
var DragListener = fc.DragListener = Class.extend({
	...
	// Call this to stop listening to the user's mouse courses
...
/* Tracks mouse movements over a CoordMap and raises courses about which cell the mouse is over.
------------------------------------------------------------------------------------------------------------------------
options:
- subjectEl
- subjectCenter
...
var Grid = fc.Grid = RowRenderer.extend({
	...
	externalDragStartProxy: null, // binds the Grid's scope to externalDragStart (in DayGrid.courses)
	...
	// Determines whether courses should have their end times displayed, if not explicitly defined by 'displayEventTime'.
	// Only applies to non-all-day courses.
	computeDisplayEventTime: function() {
		return true;
	},


	// Determines whether courses should have their end times displayed, if not explicitly defined by 'displayEventEnd'
	...
	setElement: function(el) {
		...
		el.on('mousedown', function(ev) {
			if (
				...
				!$(ev.target).closest('.fc-popover').length // not on a popover (like the "more.." courses one)
			) {
				_this.dayMousedown(ev);
			}
		});

		// attach event-element-related handlers. in Grid.courses
	...
	// DOES NOT remove any content before hand (doens't clear courses or call destroyDates), unlike View
	...
	/* Event Helper
	------------------------------------------------------------------------------------------------------------------*/
	// TODO: should probably move this to Grid.courses, like we did event dragging / resizing
	...
	fabricateHelperEvent: function(range, sourceSeg) {
		...
		// this extra className will be useful for differentiating real courses from mock courses in CSS
	...
	/* Fill System (highlight, background courses, business hours)
	------------------------------------------------------------------------------------------------------------------*/
...
Grid.mixin({
	...
	// Renders the given courses onto the grid
	...
	// Unrenders all courses currently rendered on the grid
	...
	/* Drag Rendering (for both courses and an external elements)
	------------------------------------------------------------------------------------------------------------------*/
	...
	/* Converting courses -> ranges -> segs
	------------------------------------------------------------------------------------------------------------------*/


	// Converts an array of event objects into an array of event segment objects.
	// A custom `rangeToSegsFunc` may be given for arbitrarily slicing up courses.
	...
	// Converts an array of courses into an array of "range" objects.
	// A "range" object is a plain object with start/end properties denoting the time it covers. Also an event property.
	// For "normal" courses, this will be identical to the event's start/end, but for "inverse-background" courses,
	...
	eventsToRanges: function(events) {
		...
		// group by ID so that related inverse-background courses can be rendered together
	...
	// Converts an array of "normal" courses (not inverted rendering) into a parallel array of ranges
	...
	// Converts an array of courses, with inverse-background rendering, into an array of range objects.
	// The range objects will cover all the time NOT covered by the courses.
...
function compareSegs(seg1, seg2) {
	return seg1.eventStartMS - seg2.eventStartMS || // earlier courses go first
		seg2.eventDurationMS - seg1.eventDurationMS || // tie? longer courses go first
		seg2.event.allDay - seg1.event.allDay || // tie? put all-day courses first (booleans cast to 0/1)
...
DayGrid.mixin({
	...
	// Unrenders all courses currently rendered on the grid
	destroyEvents: function() {
		this.destroySegPopover(); // removes the "more.." courses popover
	...
	renderBgSegs: function(segs) {
		...
		// don't render timed background courses
	...
	// Uses the given courses array to generate <tbody> elements that should be appended to each row's content skeleton.
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		// Only display a timed courses time if it is the starting segment
		if (seg.isStart) {
...
/* Methods relate to limiting the number courses for a given day on a DayGrid
----------------------------------------------------------------------------------------------------------------------*/
// NOTE: all the segs being passed around in here are foreground segs

DayGrid.mixin({

	segPopover: null, // the Popover that holds courses that can't fit in a cell. null when not visible
	...
	// Limits the number of "levels" (vertically stacking layers of courses) for each row of the grid.
	...
	// Reveals the popover that displays all courses within a cell
	...
	// Given the courses within an array of segment objects, reslice them to be in a single day
	resliceDaySegs: function(segs, dayDate) {

		// build an array of the original courses
		...
		// slice the courses with a custom slicing function
	...
	// Generates the text that should be inside a "more" link, given the number of courses it represents
	...
	// If `startLevel` is specified, returns only courses including and below that level. Otherwise returns all segs.
...
TimeGrid.mixin({
	...
	// Repositions business hours segs too, so not just for courses. Maybe shouldn't be here.
	...
	generateSegPositionCss: function(seg) {
		...
		if (shouldOverlap && seg.forwardPressure) {
			// add padding to the edge so that forward stacked courses don't cover the resizer's icon
...
var View = fc.View = Class.extend({
	...
	// Does every type of rendering EXCEPT rendering courses.
	...
	// Clears dates and courses. Does not clear the skeleton.
	...
	// Does everything necessary to display the given courses onto the current view
	...
	// Does everything necessary to clear the view's currently-rendered courses
	...
	// Renders the courses onto the view.
	renderEvents: function(events) {
		// subclasses should implement
	},
	...
	// Signals that all courses have been rendered
	...
	// Basically runs courses and elements through the eventRender hook.
	...
	reportEventDrop: function(event, dropLocation, largeUnit, el, ev) {
		...
		calendar.reportEventChange(); // will rerender courses
	},
	...
	/* Drag-n-Drop Rendering (for both courses and external elements)
	------------------------------------------------------------------------------------------------------------------*/
...
function Calendar_constructor(element, overrides) {
	...
	function windowResize(ev) {
		if (
			!ignoreWindowResize &&
			ev.target === window && // so we don't process jqui "resize" courses that have bubbled up
	...
	function refetchEvents() { // can be called as an API method
		destroyEvents(); // so that courses are cleared before user starts waiting for AJAX
		fetchAndRenderEvents();
	}


	function renderEvents() { // destroys old courses if previously rendered
		if (elementVisible()) {
			freezeContentHeight();
...
function EventManager(options) { // assumed to be a calendar
	...
	var loadingLevel = 0;
	var cache = []; // holds courses that have already been expanded
				...
				if (eventInputs) {
					for (i = 0; i < eventInputs.length; i++) {
						...
						if (abstractEvent) { // not false (an invalid event)
							cache.push.apply(
								cache,
								expandEvent(abstractEvent) // add individual expanded courses to the cache
							);
						}
					}
				}
	...
	function removeEventSource(source) {
		...
		// remove all client courses from that source
	...
	// Returns a hash of misc event properties that should be copied over to related courses.
	...
	// returns the expanded courses that were created
	...
	function removeEvents(filter) {
		...
		if (filter == null) { // null or undefined. remove all courses
		...
		// Remove courses from array sources.
	...
	// Modifies an event and all related courses by applying the given properties.
	...
	function mutateEvent(event, newProps, largeUnit) {
		...
		// apply the operations to the event and all related courses
		undoFunc = mutateEvents(
			clientEvents(event._id), // get courses with this ID
			clearEnd,
			newProps.allDay,
			startDelta,
			durationDelta,
	...
	// Modifies an array of courses in the following ways (operations are in order):
	...
	//
	// TODO: don't use so many closures. possible memory issues when lots of courses with same ID.
	//
	...
	// Returns an array of courses as to when the business hours occur in the given view.
	...
	function isRangeAllowed(range, constraint, overlap, event) {
		...
		// the range must be fully contained by at least one of produced constraint courses
		if (constraint != null) {
...
// Returns a list of courses that the given event should be compared against when being considered for a move to
...
Calendar.prototype.getPeerEvents = function(event, range) {
	...
	for (i = 0; i < cache.length; i++) {
		...
		if (
			!event ||
			event._id !== otherEvent._id // don't compare the event to itself or other related [repeating] courses
		) {
...
var BasicView = fcViews.basic = View.extend({
	...
	numberCellHtml: function(cell) {
		...
		if (!this.dayNumbersVisible) { // if there are week numbers but not day numbers
			return '<td/>'; //  will create an empty space above courses :(
		}
	...
	// Renders the given courses onto the view and populates the segments array
	renderEvents: function(events) {
		this.dayGrid.renderEvents(events);

		this.updateHeight(); // must compensate for courses that overflow the row
	},
	...
	destroyEvents: function() {
		...
		// B) in IE8, this causes a flash whenever courses are rerendered
	},


	/* Dragging (for both courses and external elements)
	------------------------------------------------------------------------------------------------------------------*/
...
var AgendaView = fcViews.agenda = View.extend({
	...
	setHeight: function(totalHeight, isAuto) {
		...
		// limit number of courses in the all-day area
		if (this.dayGrid) {
			this.dayGrid.destroySegPopover(); // kill the "more" popover if displayed
	...
	// Renders courses onto the view and populates the View's segment array
	renderEvents: function(events) {
		var dayEvents = [];
		var timedEvents = [];
		var daySegs = [];
		...
		// separate the courses into all-day and timed
		for (i = 0; i < events.length; i++) {
			if (events[i].allDay) {
		...
		// render the courses in the subcomponents
		timedSegs = this.timeGrid.renderEvents(timedEvents);
		if (this.dayGrid) {
			daySegs = this.dayGrid.renderEvents(dayEvents);
		}

		// the all-day area is flexible and might have a lot of courses, so shift the height
		this.updateHeight();
	},
	...
	destroyEvents: function() {
		...
		// destroy the courses in the subcomponents
	...
	/* Dragging (for courses and external elements)
	------------------------------------------------------------------------------------------------------------------*/
