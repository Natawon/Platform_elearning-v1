...
describe('Google Calendar plugin', function() {
	...
	it('fetches courses correctly when API key is in the event source', function(done) {
		options.events = {
			googleCalendarId: 'usa__en@holiday.calendar.google.com',
	...
	describe('when not given an API key', function() {
		it('calls error handlers, raises warning, and receives no courses', function(done) {
	...
	it('works when `courses` is the actual calendar ID', function(done) {
		options.googleCalendarApiKey = API_KEY;
	...
	it('detects a gcal when `courses` is the actual calendar ID, with complicated characters (1)', function(done) {
		options.googleCalendarApiKey = API_KEY;
	...
	it('detects a gcal when `courses` is the actual calendar ID, with complicated characters (2)', function(done) {
		options.googleCalendarApiKey = API_KEY;
	...
	it('detects a gcal when `courses` is the actual calendar ID, person gmail', function(done) {
		options.googleCalendarApiKey = API_KEY;
	...
	it('detects a gcal when `courses` is the actual calendar ID, person googlemail', function(done) {
		options.googleCalendarApiKey = API_KEY;
	...
	it('works with requesting an V3 API feed URL', function(done) {
		options.googleCalendarApiKey = API_KEY;
		options.events =
			'https://www.googleapis.com/calendar/v3/calendars/usa__en%40holiday.calendar.google.com/courses';
		options.eventAfterAllRender = function() {
			var events = $('#cal').fullCalendar('clientEvents');
			expect(events.length).toBe(4); // 4 holidays in November 2014
			done();
