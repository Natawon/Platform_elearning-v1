describe('removeEventSource', function() {
	...
	function testInput(eventInput) {
		...
		it('correctly removes courses provided via `courses` at initialization', function(done) {
			var callCnt = 0;
		...
		it('correctly removes courses provided via `eventSources` at initialization', function(done) {
			var callCnt = 0;
			options.eventAfterAllRender = function() {
				if (!(callCnt++)) { // only the first time
					expectEventCnt(2);
		...
		it('correctly removes courses provided via `addEventSource` method', function(done) {
			var callCnt = 0;
			options.eventAfterAllRender = function() {
