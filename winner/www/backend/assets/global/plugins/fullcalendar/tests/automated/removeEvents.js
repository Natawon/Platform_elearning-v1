describe('removeEvents', function() {
	...
	$.each({
		...
		describe(description, function() {
			...
			it('can remove all courses if no args specified', function(done) {
				go(
					events,
					function() {
			...
			it('can remove courses with a numeric ID', function(done) {
				go(
					events,
					function() {
			...
			it('can remove courses with a string ID', function(done) {
				go(
					events,
					function() {
						$('#cal').fullCalendar('removeEvents', '1');
			...
			it('can remove courses with a filter function', function(done) {
				go(
					events,
					function() {
	...
	function go(events, removeFunc, checkFunc, doneFunc) {
		...
		options.eventAfterAllRender = function() {
			if (!called) { // don't execute on subsequent removeEvents/next/prev
				...
				checkAllEvents(); // make sure all courses initially rendered correctly
				removeFunc(); // remove the courses
	...
	// Checks to make sure all courses have been rendered and that the calendar
	// has internal info on all the courses.
