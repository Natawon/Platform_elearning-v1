...
$categories2courses = categories2courses($_GET['id'], $groupKey, [
    'page' => $_GET['page'],
]);
...
usort($categories2courses['courses']['data'], "desc_sort_json");
...
<head prefix="og: http://ogp.me/ns# fb: http://ogp.me/ns/fb# website: http://ogp.me/ns/fb/website#">
    <!-- Meta -->
    <meta charset="utf-8">
    <title><?=$categories2courses['categories']['title']?> - <?=$configuration['title']?></title>
...
<section id="content-list-page">
    <div class="row">
        <h2 class="text-center col-md-12 col-xs-12 title-page <?=$categories2courses['categories']['css_class']?>"><?=$categories2courses['categories']['title']?></h2>
    </div>
<div class="container">
    <?php if ($groupKey == 'SETTFEX') { ?>
    <div class="row">
        <div class="col-xs-12">
            <div class="wrapper-filter m-b-15">
                <strong class="">
                    Filter:
                </strong>
                <div class="btn-group m-l-5" role="group" aria-label="...">
                    <button type="button" class="btn btn-default f-bold f-18 p-x-12 p-y-2 <?=($filter_state == 'all') ? 'active' : ''?>" onclick="window.location='?state=all'">All</button>
                    <button type="button" class="btn btn-default f-bold f-18 p-x-12 p-y-2 <?=($filter_state == 'live') ? 'active' : ''?>" onclick="window.location='?state=live'">Live</button>
                    <!-- <button type="button" class="btn btn-default f-bold f-18 p-x-12 p-y-2 <?=($filter_state == 'upcoming') ? 'active' : ''?>" onclick="window.location='?state=upcoming'">Upcoming</button> -->
                    <button type="button" class="btn btn-default f-bold f-18 p-x-12 p-y-2 <?=($filter_state == 'vod') ? 'active' : ''?>" onclick="window.location='?state=vod'">VOD</button>
                </div>
    ...
    <div class="row">
        <div class="col-md-12">
            <?=$categories2courses['categories']['description']?>
        </div>

        <?php foreach($categories2courses['courses']['data'] as $rs_courses_list){ ?>
            <div class="col-lg-3 col-md-3 col-sm-4">
                <div class="col-item <?=(!$rs_courses_list['topics']) ? 'col-item-vod' : ''?> <?=$rs_courses_list['category']['css_class']?>">
                    <div class="photo">
                        ...
                        <div class="state-status state-status-<?=($rs_courses_list['topics']['streaming_status'] == 1) ? 'danger' : 'info'?>">
                    ...
                    <div class="info">
                        ...
                        <div class="separator clearfix">
                            <p class="btn-add"><?=$rs_courses_list['category']['title']?></p>
    ...
    <div class="row">
        <div class="col-md-12 text-center">
            <?php
            if (!empty($categories2courses['courses']['data'])) {
                echo $oFunc->createPagination($oFunc->getCurrentURL(), $categories2courses['courses']['total'], $categories2courses['courses']['per_page'], 'pagination', '', '<span aria-hidden="true">&laquo;</span> ย้อนกลับ', 'ถัดไป <span aria-hidden="true">&raquo;</span>');
            }
            ?>
        </div>
    </div>
